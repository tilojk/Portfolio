...
const ContactSection = () => {
  ...
  return (
    <section id="contact" className='relative w-full min-h-screen bg-neutral-6 overflow-hidden'>
        <h2 className="relative text-4xl md:text-5xl pt-16 mb-12 md:mb-16 2xl:mb-20 text-center z-15">
            <b>Kontakt</b>
        </h2>
        <div className='relative w-full flex flex-col md:flex-row px-4 md:px-12 xl:px-24 gap-12 lg:gap-20 z-10'>
            <div className='md:flex-1'>
                ...
                <Link href='mailto:tilo@v-jaekel.de' className='text-secondary-1 hover:underline'>tilo@v-jaekel.de</Link>
            </div>
            <div className='md:flex-1'>
                <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
                    ...
                    <div>
                        ...
                        <label htmlFor="privacy" className="text-white pl-2 mb-2 text-base font-medium">Ich habe die <a href="/datenschutz" target="_blank" className="text-secondary-1 hover:underline">Datenschutzerklärung</a> gelesen und akzeptiere sie.</label>
                    </div>
                    <button type="submit" className="py-2.5 px-4 bg-secondary-2/80 backdrop-blur-xl hover:bg-secondary-2 transition-all duration-150 rounded-xl cursor-pointer w-fit">Absenden</button>
                </form>
            </div>
        </div>
        <div className="w-full h-80 md:min-h-screen md:absolute top-0 left-0 flex justify-center items-center mt-16 md:mt-auto">
            <div ref={imageRef} className="absolute max-h-1/2 md:max-h-4/5 lg:max-h-3/4 w-80 h-80 lg:w-96 lg:h-96 2xl:w-108 2xl:h-108 scale-x-[-1] -bottom-2 -left-20 md:-left-8 lg:left-24 z-5">
            ...
            <div className="bg-radial from-secondary-1 to-transparent rounded-full h-128 w-128 z-0 blur-lg absolute top-full right-0 transform translate-x-1/2 -translate-y-1/2"></div>
        </div>
    </section>
  )
}
